...
if [[ -n "${EXISTING_TWG}" ]]; then
  ...
  if [[ -n "${EXISTING_VERSION}" && "${EXISTING_VERSION}" != "${VERSION}" ]]; then
    echo ""
    echo "This Claude Code plugin was built for TWG v${VERSION}. If your installed CLI or skills are stale, run:"
  ...
  echo "  twg doctor"
  echo ""
  echo "If Claude Code does not see TWG skills after setup or update, run:"
  echo "  twg skills install --agent claude --yes"
  echo "Then start a new Claude Code session."
  exit 0
fi

echo "TWG CLI is not installed on PATH."
echo "This Claude Code plugin delegates setup to the hosted TWG installer so every agent and the terminal share one binary and one set of skills."
...
if command -v twg >/dev/null 2>&1; then
  ...
  twg doctor || true
  echo ""
  echo "If TWG skills were installed or updated, start a new Claude Code session."
else
