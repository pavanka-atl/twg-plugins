...
name: twg-setup
description: >
  Atlassian Teamwork Graph (TWG) gives Codex grounded Atlassian work
...
Goal: keep the `twg` binary current, authenticated, and healthy for Codex.
...
When setup is healthy, suggest a few useful TWG prompts for Codex. Let the user pick the first work-data request to run.
