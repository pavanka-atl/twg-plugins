...
name: twg-setup
description: >
  Install, update, authenticate, or repair `twg` for your coding agent, including missing CLI
  or skills and doctor/auth follow-up. TWG gives your coding agent grounded work context across Jira, Confluence, Bitbucket, JSM, Assets, Slack,
...
# TWG Setup for your coding agent

Use the installed `twg` CLI for your coding agent. This plugin ships no binary, wrapper, or local launcher.
...
5. If your coding agent does not see TWG skills, or doctor reports a skill issue, refresh
   them at the standard agent skill location:
...
reload or start a new agent session so new skills load.
...
Only when the user's request was setup alone, offer a few useful TWG prompts for your coding agent:
